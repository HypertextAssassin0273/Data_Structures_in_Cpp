...
public:
	DCLL()noexcept://default ctor
		tail(nullptr), _size(0){}
	
	DCLL(const DCLL &other)noexcept://copy ctor
		tail(nullptr), _size(0){
    ...
    DCLL& operator=(const DCLL &other)noexcept{//copy assignment func.
...
#if __cplusplus >= 201103L
 	DCLL(DCLL&& other)noexcept://move ctor (C++11 Construct)
  		tail(other.tail), _size(other._size){//1) steal other's data
		other.tail=nullptr; other._size=0;//2) set 'other' to null state
	}
	DCLL& operator=(DCLL&& other)noexcept{//move assignment func (C++11 Construct)
	...
	DCLL(_T (&&init_list)[N])noexcept://move initializer_list ctor
		tail(nullptr), _size(0){
	...
	*/
	...
	void erase(size_t pos){//removes node randomly
		if(!tail) return;
	...
	void merge(DCLL &other, __int64 pos=0){//TAIL CONDITION NOT SATISFIED...
