...
public:
	...
	const node* back_node()const{ return tail; }
	...
	template<bool=true>//Note: use it when datatype is integer in order to avoid using overloaded version
	void erase(size_t index=0){//removes given index position node in O(n) complexity
    	if(!index){ pop_front(); return; }
	...
	node* erase(size_t index, bool){
		if(!index) return pop_front(true);
		if(index==_size-1){ pop_back(true); return; }
