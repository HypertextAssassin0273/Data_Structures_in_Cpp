...
#if __cplusplus >= 201103L
	//reversely traverse fixed-size deque using these methods (inside range-based loop):
	reverse_iterator_adapter<__FD, r_iterator> r_iterator_adapter(){ 
		return reverse_iterator_adapter<__FD, r_iterator>(*this);
	}
	reverse_iterator_adapter<const __FD, const_r_iterator> r_iterator_adapter()const{
		return reverse_iterator_adapter<const __FD, const_r_iterator>(*this);
	}
#endif
