...
public:
	...
	void push_front(type&&... attrbs){//constructs element (directly) at first position [complexity: O(1)]
		...
		//constructing mapped_object format: new(targeted_memory_address) type_constructor();
	}
	template<typename... type>
...
private:
	...
	public:
		...
		/* (additional) accessor */
		_T& operator[](size_t i)const{ return ptr[(elem_idx+i)%base::cap]; }
		
		/* modifiers (arithmetic operators) */
		//prefix operators:
		__self& operator++(){ ++iter_idx; ++elem_idx; return *this; }
		__self& operator--(){ --iter_idx; --elem_idx; return *this; }
		
		//postfix operators:
		__self operator++(int){ __self temp(*this); ++iter_idx; ++elem_idx; return temp; }
		__self operator--(int){ __self temp(*this); --iter_idx; --elem_idx; return temp; }
		
		__self& operator+=(size_t i){ iter_idx+=i; elem_idx+=i; return *this; }
		__self& operator-=(size_t i){ iter_idx-=i; elem_idx-=i; return *this; }
		
		__self operator+(size_t i)const{ return __self(ptr, elem_idx+i, iter_idx+i); }
		__self operator-(size_t i)const{ return __self(ptr, elem_idx-i, iter_idx-i); }
		
		friend __self operator+(size_t i, const __self& other){
			return __self(ptr, other.elem_idx+i, other.iter_idx+i);
		}
		
		__int64 operator-(const __self& other)const{ return iter_idx-other.iter_idx; }
	...
	public:
		...
		__int64 operator-(const __self& other)const{ return other.iter_idx-iter_idx; }
