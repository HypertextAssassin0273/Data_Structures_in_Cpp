...
tree_adapter_type get_MAATA_obj_method_1(){
	...
	return obj;//std::move(obj) => not req. bcz of R.V.O
}

tree_adapter_type get_MAATA_obj_method_2(){
	int i=393; String s{"copied"};
	
	return tree_adapter_type({//using initializer_list ctor
...
void basic_operations(tree_adapter_type& obj){
	//bool,int,String,double,char,String
	// 0    1    2      3     4     5
...
//	obj.search(INDEX,x,'F');//l-values can be safely passed in any parameter (no compile-time variables needed)
	
	cout<<"\n-----\nSearch all \"OXO\" after removing {\"OXO\",111.111,'C'} :-\n";
	obj.remove(2, String{"OXO"},111.111,'C');//remove specific records by providing multiple attributes
...
//	obj.remove({1, 3}, String{"hello"}, 444.444);NEED TO ADD THIS OVERLOAD VERSION
	obj.remove(2, String{"hello"},444.444);
	...
	obj.search(1, 321,String{"stranded"});
