...
//	obj.search({1,2}, 321,String{"stranded"});

	cout<<"\n-----\nSort by Atrribute [int: 1]:-\n";
//	int index=1;
//	obj.sort(index);//run-time
	obj.sort<2>();//compile time
	
	cout<<"\n-----\nUpdate all {321,\"stranded\"}:-\n";
	auto dfl=obj.search({1,2}, 321,String{"stranded"});
	for(auto& it:dfl) cout<<it->data<<'\n';
	obj.update(std::move(dfl), {3,2}, 789.76,String{"yowza!"});
//	print_type<decltype(obj.search({1,2}, 99999,String{"stranded"}))>();
