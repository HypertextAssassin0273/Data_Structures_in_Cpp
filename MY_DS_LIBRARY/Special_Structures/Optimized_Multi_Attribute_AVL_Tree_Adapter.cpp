...
typedef MAATA_O<bool,int,String,double,char,String> tree_adapter_type;

tree_adapter_type get_MAATA_O_obj_method_1(){
...
tree_adapter_type get_MAATA_O_obj_method_2(){
	int i=393; String s{"copied"};
...
////	int indexes[]{1, 3}; obj.search(indexes, i,d);//l-value indexes array is also accpeted
//	obj.view_filtered_records(obj.search({1, 3}, i, Make_CA(d)));//l-values can be safely passed in any parameter
	
//	cout<<"\n-----\nSearch all \"OXO\" after removing {\"OXO\",111.111,'C'} :-\n";
//	obj.remove({2,3,4}, String{"OXO"},
//			   Make_CA(111.111, greater_equal),
//			   Make_CA('C'));//remove specific records by providing multiple attributes along with random filters
//	obj.view_filtered_records(obj.search({2}, String{"OXO"}));//displays all respective matched records
	
//	cout<<"\n-----\nSearch all \"hello\" after removing {\"hello\",444.444} :-\n";
////	obj.remove({3, 5}, 444.444, Make_CA(String{"stranded"}));
////	obj.remove({2, 3}, String{"hello"}, Make_CA(444.444));
//	obj.remove({3, 2}, 444.444, Make_CA(String{"hello"}));//can be used in any order
//	obj.view_filtered_records(obj.search({2}, String{"hello"}));
	
//	cout<<"\n-----\nSearch by {321,\"stranded\"} after inserting {true,321,\"hello\",345.1,'c',\"n.i.l\"} :-\n";
//	obj.insert(true,321,String{"hello"},345.1,'c',String{"n.i.l"});
//	obj.view_filtered_records( obj.search({1,2}, 321, Make_CA(String{"stranded"})) );
	
//	cout<<"\nData Sorted by Atrribute: {int: 1}\n";
////	int index=1; obj.sort(index);//run-time
//	obj.sort<4>();//compile time
	
//	cout<<"\n-----\nUpdate all {321,\"stranded\"}:-\n";
//	auto dfl=obj.search({1,2}, 321, Make_CA(String{"stranded"}));
//	obj.view_filtered_records(dfl);
//	obj.update(std::move(dfl), {3,2}, 789.76,String{"yowza!"});
	
//	cout<<"\n-----\nSearch Indexes by {'f',\"OXO\"}:-\n";
////	obj.view_filtered_records(obj.search({5,7}, {-1}));//specific indexes search
//	obj.view_filtered_records(obj.search({5,7}, {4,2}, Make_CA('f'), Make_CA(String{"OXO"})));//filtered indexes search
...
void file_operations(tree_adapter_type& obj){
	...
	obj.fwrite("df_3.csv");//overwrites df_3.csv file if exists
}

void traverse_data(tree_adapter_type& obj){
	cout<<"RECORDS:\n";
//	cout<<"Indexer-View:\n";
//	obj.view_records(true);
	cout<<"\nList-View:\n";
	obj.view_records();//bool version available for indexer view
	cout<<"\nTrees-View:\n";//Note: always prefer to use '\n' over std::endl
	obj.view_by_trees();
}

int main(){
//	auto obj=get_MAATA_O_obj_method_1();//by multiple emplaced_inserts
//	auto obj=get_MAATA_O_obj_method_2();//by initializer_list
	auto obj=get_MAATA_O_obj_method_3();//by file-stream
