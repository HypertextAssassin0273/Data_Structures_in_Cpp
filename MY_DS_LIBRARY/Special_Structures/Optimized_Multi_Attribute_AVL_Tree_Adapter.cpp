...
//	obj.update(std::move(dfl), {3,2}, 789.76,String{"yowza!"});
	...
	for(auto& it:obj.search({5,7}, {-1})) cout<<it->data.second<<'\n';//specific indexes search
//	for(auto& it:obj.search({5,7}, {4,2}, 'f',String{"OXO"})) cout<<it->data.second<<'\n';//filtered indexes search
...
void traverse_data(tree_adapter_type& obj){
	cout<<"RECORDS:\nIndexer-View:\n";
	obj.traverse_by_records(true);
	cout<<"\nList-View:\n";
	obj.traverse_by_records();//bool version available for indexer view
