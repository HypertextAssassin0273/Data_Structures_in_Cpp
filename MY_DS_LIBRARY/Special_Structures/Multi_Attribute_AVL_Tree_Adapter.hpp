...
class MAATA{
	...
	private:
		...
		}//returns a ptr to forward_list of duplicate record node ptrs (on successful search)
...
public:
	...
	void remove(const size_t key_index,		 //by filtering key-attributes (in specific order)
		...
		if(dup_list_ptr){//null-exception safety
			auto iter=dup_list_ptr->front_node();//node iterator for traversing dup_list
			typename list_type::node* dup_node_ptr;//duplicate record node pointer
			bool is_present;//flag for storing decision whether to keep duplicate or not
			size_t index;//index ptr for traversing record attributes
			...
			while(iter){
				...
				if(is_present){//removes only respective matched records
					...
					this->data.erase(dup_node_ptr);//removes random node from list in O(1) time
				}
			}
		}
	...
	void search(const size_t key_index,		  //by filtering key-attributes (in specific order)
		...
		if(dup_list_ptr){//for null-exception safety
			...
			for(const auto& iter:*dup_list_ptr){//using ptr to forward_list of duplicate record node ptrs
				index=key_index+1; is_present=true;//need to reset for every record
