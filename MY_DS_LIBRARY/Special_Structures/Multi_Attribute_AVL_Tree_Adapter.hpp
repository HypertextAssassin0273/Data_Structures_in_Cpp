...
public:
	...
	void insert(__attributes&&... key_record_attrs){//emplaced_insert, efficient version
		...
		bool expander[]{
			( ((e_AVL<attributes, typename list_type::node>*)(this->trees+idx++))->
	...
	void remove(const int (&key_indexes)[N], const type& key, const types&... keys){
		...
		auto dup_list_ptr=((e_AVL<type, typename list_type::node>*)(this->trees+key_indexes[0]))->search(key);
		if(dup_list_ptr){//null-exception safety
			...
			while(begin!=end){
				...
				if(is_present){//removes only respective matched records
					...
					this->data.erase(dup_node_ptr);//removes current node (along with its record) from list in O(1) time
				}
			}
	...
	void clear(){//clears all the resources of both list & trees
		...
		size_t idx=0;
		bool expander[]{
			(((e_AVL<attributes, typename list_type::node>*)(this->trees+idx++))->clear(), false)...
		};
	...
	void search(const int (&key_indexes)[N], const type& key, const types&... keys)const{
		...
		if(dup_list_ptr){//for null-exception safety
			size_t idx;//index iterator for traversing key_indexes (of respective record attributes)
			bool is_present;//flag for storing decision whether to display duplicate or not
	...
	void traverse_by_attributes()const{//traverse records by trees
		...
		size_t idx=0;
		bool expander[]{
			( ((e_AVL<attributes, typename list_type::node>*)(this->trees+idx++))->traverse(),
				std::cout<<'\n', false )...
		};
	}
