...
public:
	...
	template<size_t N1, size_t N2, typename... types>//overload for applying other operations on filtered records
	dup_list_t search(const int (&record_indexes)[N1], const int (&key_indexes)[N2], const types&... keys)const{
		if(!N1||N1>indexer.size()||N2!=sizeof...(keys)&&sizeof...(keys)) return dup_list_t();//bound exception safety
		dup_list_t filtered_dup_list;
		size_t idx; bool is_present;
		for(auto& i:record_indexes){
			idx=0; is_present=true;
			bool expander[]{ is_present=is_present&&(keys==*(types*)(indexer[i]->data.second+key_indexes[idx++]))... };
			if(is_present) filtered_dup_list.push_front(indexer[i]);//stores only respective matched record nodes
		}
		return filtered_dup_list;
