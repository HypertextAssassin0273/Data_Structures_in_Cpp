...
//Deduce Nth type from Template Parameter-Pack (at Compile Time):
template<size_t, typename...>
struct __nth_type;//forward declaration of primary template struct

template<typename T0, typename... Ts>//T0: 1st type, Ts: (remaining all) types
struct __nth_type<0, T0, Ts...>{//partially specialized template struct (base-case for terminating condition)
    typedef T0 type;
};
template<size_t N, typename T0, typename... Ts>//N: (remaining) number count
struct __nth_type<N, T0, Ts...>{//main template struct
    typedef typename __nth_type<N-1, Ts...>::type type;//template recursion in struct
};

template<size_t N, typename... Ts>
using nth_type=typename __nth_type<N, Ts...>::type;//helper type (for simplified name)
