...
#include"../Node_Structures/Forward_List.hpp"
...
/* Extended AVL class for handling Pair of respective pointed types alongwith their duplicate
   record node-addresses in singly-list stack, only for multi_attribute_tree_adapter class */
template<typename T,typename list_node_type>
class special_AVL:public AVL<Pair<T*,Forward_List<list_node_type*>>>{
	typedef Forward_List<list_node_type*> f_list_type;
	typedef Pair<T*,f_list_type> pair_type;
	typedef AVL<pair_type> base;
	using base::base;//i.e. inheriting all ctors of base class
	
public:
	void insert(T* first,list_node_type* second){
		base::root=insert(Pair<T*,list_node_type*>(first,second),base::root); 
	}
	void remove(const T& key){ base::root=remove(key,base::root); }
	f_list_type* search(const T& key)const{ return search(key,base::root); }
	void traverse()const{ in_order_traversal(base::root); }
	
private:
	typename base::node* insert(const Pair<T*,list_node_type*>& key,typename base::node* current){
		if (!current){
			++base::_size;
			return new typename base::node(pair_type(key.first,f_list_type{key.second}));
		}
		if (*key.first==*current->key.first){
        	current->key.second.push_front(key.second);//i.e. inserting node of respective duplicate in stack of record-addressses
        	++(current->count);
        	return current;
    	}
		if (*key.first<*current->key.first)
	...
	typename base::node* remove(const T& key,typename base::node* current){
		...
		if (key<*current->key.first)
			...
			if(current->count>1){
				cout<<*current->key.second.front()<<' ';
			//	current->key.second.pop_front();//i.e. releasing node of respective duplicate from stack of record-addressses
				--(current->count);
				return current;
			}
	...
	f_list_type* search(const T& key,typename base::node* current)const{
		if(!current)
			return nullptr;
        if(key<*current->key.first)
	...
	void in_order_traversal(typename base::node *current)const{
		if (current){
			...
			std::cout<<*current->key.first<<'('<<current->count<<" : "<<current->key.second.size()<<") ";
...
public:
	...
	void insert(_attributes&&... key_record){//i.e. O(M*logN) time-complexity [here, N=total records, M=total attributes]
		...
		bool expander[]{
			( ((special_AVL<attributes,typename list_type::node>*)(trees+index++))->
				insert((attributes*)(data.back()+index),data.back_node()),
				false )... 
		};
	}
	//FEATURE: need move_insert_for_indexed_struct, no copy!!
														//not ready//
	template<typename T>
	void remove(const T& key,const size_t index){//i.e. O((M+D)*logN) time-complexity [here, D=total duplicates]
		Forward_List<typename list_type::node*>* FL_ptr=((special_AVL<T,typename list_type::node>*)(trees+index))->search(key);
		if(FL_ptr){//i.e. to handle null-exception for both AVL-tree & List containers
			for(const auto& it:*FL_ptr)
				cout<<*it.data<<'\n';
			size_t i=0;
			
//			cout<<*FL_ptr->front();//*(*FL_ptr)[0]

			bool expander[]{
				( ((special_AVL<attributes,typename list_type::node>*)(trees+i++))->
					remove(*(attributes*)(FL_ptr->front()->data+i)),cout<<'\n',
					false )...
			};

//			data.erase(FL_ptr->front());//i.e. removes node randomly in O(1) time when current_node is known
			//cout<<*FL_ptr->front();
		}
		else
	...
	template<typename T>
	void search(const T& key,const size_t index)const{//i.e. O(logN) time-complexity [here, M & D cancels out each other]
		Forward_List<typename list_type::node*>* FL_ptr=((special_AVL<T,typename list_type::node>*)(trees+index))->search(key);
		if(FL_ptr)//i.e. for null-exception safety
			for(const auto& it:*FL_ptr)//i.e. prints all respective records of key (duplicates)
				cout<<*it.data<<'\n';
		else
			cout<<"No Records Found!\n\a";
	}
