...
class Vector{
	...
	u_char *data;//i.e. data-buffer on heap
public:
	...
	Vector(size_t n=0,_T&&... val)://i.e. default & emplaced fill ctor
		_size(0),_capacity((n>max_capacity)?throw:n),
		data(new u_char[sizeof(T)*_capacity]){//i.e. allocating new data-buffer
...
#else
	Vector(size_t n=0,const T& val=T())://i.e. default & fill ctor
	...
	Vector(const std::initializer_list<T>& list)noexcept://i.e. ctor for initializer-list
		_size(0),_capacity(list.size()),data(new u_char[sizeof(T)*_capacity]){
...
#endif
	Vector(u_char *new_data,size_t new_size,size_t new_capacity)noexcept://i.e. special move ctor
 	...
 	/*i.e. Accessors */
	T& operator[](size_t n)const{//i.e. gives read & write both access
...
private:
	void reallocate(size_t n){
		...
		#else
			new(new_data+sizeof(T)*i) T(*(T*)(data+sizeof(T)*i));//i.e. copy constructing obj to 'new_data' (slower process)
		#endif
...
#if __cplusplus >= 201103L
	template<typename... _T>
	void resize(size_t n,_T&&... val){//i.e. emplaced_resize
