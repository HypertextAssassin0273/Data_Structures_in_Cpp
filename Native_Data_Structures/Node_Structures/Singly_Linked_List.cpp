...
void test_case_1(){
	...
	Forward_List<int,true> list1={1,2,4,5},list2;
#else
    Forward_List<int,true> list1,list2;
	list1.push_back(1);
	list1.push_back(2);
	list1.push_back(4);
	list1.push_back(5);
	...
	Forward_List<int,true> list3=list2;
    cout<<"List2:"<<'\n';
	list2.traverse();
	cout<<"List3 (int):"<<'\n';
	list3.traverse();
...
#if __cplusplus >= 201103L
	Forward_List<int,true> list4={55,66,77,88,99};
#else
    Forward_List<int,true> list4;
	...
	list3.clear();
	list4.clear();
	Forward_List<int,true>::clear_gc();//i.e. releasing all nodes of int type
...
int main(){
	Forward_List<int> list{1,2,3,4},list2={22,33,44};
	cout<<list<<'\n'<<list2<<'\n';
	list.merge(list2,1);
	cout<<list<<'\n'<<list2;
//	test_case_1();//use of all operations with GC version 
	test_case_2();//using Forward_List as stack with LIFO order (best-case, standard version)
    return 0;
}
