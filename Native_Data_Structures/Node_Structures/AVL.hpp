...
protected:
	...
	node* insert(const T& key,node* current){
		...
		__int64 bf=balance_factor(current);
  		if (bf>1)
    		if (key<current->left->data)//i.e. left left case
      			return right_rotate(current);
    		else if(key>current->left->data){//i.e. left right case
      			current->left=left_rotate(current->left);
      			return right_rotate(current);
    		}
  		if (bf<-1)
    		if (key>current->right->data)//i.e. right right case
      			return left_rotate(current);
    		else if (key<current->right->data){//i.e. right left case
	...
	node* min_value_node(node *current){//i.e. returns the inorder successor
		if(current)
			while(current->left)//i.e. finds the left-most leaf
	...
	node* remove(const T& key,node* current){
		...
		if (key<current->data)
			...
			if (!current->left||!current->right){//i.e. if the node is with only one child or no child
				...
				#else
					*current=*temp;//i.e. copy-assignment of node
				#endif
				--_size;
				delete temp;
			}
				...
				current->data=temp->data;//i.e. data of min_value_node can't be moved, only copy assignment is possible
