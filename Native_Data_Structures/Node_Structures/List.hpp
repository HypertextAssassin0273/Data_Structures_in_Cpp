...
protected:	
	...
	__uint32 _size;
public:
	List(const __uint64& n=0)noexcept:head(nullptr),tail(nullptr),_size(0){//i.e. default ctor
		if(n<=max_capacity)
			while(n>_size)
           		push_back(T());
	}
#if __cplusplus >= 201103L
	template<typename... _T>
	List(const __uint64& n,_T&&... new_data)noexcept:
	...
	void insert(const __uint64& index,node* new_node){
...
#if __cplusplus >= 201103L
	template<typename... _T>
	void resize(const __uint64& new_size,_T&&... new_data){//i.e. emplaced_resize
#else
	void resize(const __uint64& new_size,const T& new_data=T()){
#endif
...
public:
	...
	void sorted_insert(T&& new_data,bool direction_flag=true){
		sorted_insert(GC.ptr?GC.pop(forward<T>(new_data)):new node(forward<T>(new_data)),direction_flag);
	}
#endif
	...
	void merge(List &other,const __uint64& pos=0){
