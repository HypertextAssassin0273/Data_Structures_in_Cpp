...
public:
	...
	void push_front(_T&&... new_data){//i.e. emplace_front
		node *new_node=new node(forward<_T>(new_data)...);//i.e. direct initialization is possible
#else
...
#if __cplusplus >= 201103L
	template<typename... _T>
	void insert(const __int64& pos,_T&&... new_data){//i.e. emplace_randomly
#else
	void insert(const __int64& pos,const T& new_data){
#endif
		if(!tail)
        	return;
    	if(pos<2){
    	#if __cplusplus >= 201103L
    		push_front(forward<_T>(new_data)...);
		#else
			push_front(new_data);
    	#endif
			return;
    ...
    #if __cplusplus >= 201103L
		temp->next=new node(forward<_T>(new_data)...);
	#else
	...
	void erase(const __int64& pos){//i.e. removes node randomly
		if(!tail)
        	return;
    	if(pos<2){
    		pop_front();
