	...
	Vector& operator=(const Vector& other)noexcept{//i.e. copy assignment func
 	...
 	Vector(Vector&& other)noexcept://i.e. move ctor (C++11 Construct)
		...
		other.data=nullptr;//2) set other's ptr to null state
  		other._size=0; other._capacity=0;
...
public:
#if __cplusplus >= 201103L
	void push_back(T&& val){//i.e. useful for moving register values/objects
		if(_size>=_capacity)
...
#else
	void push_back(const T& val){//i.e. copies register values/objects
    	if(_size>=_capacity)
			reallocate(_capacity?_capacity*=2:++_capacity);
    	new(data+sizeof(T)*_size++) T(val);
	}
#endif
	void push_back(T& val){//i.e. copies values/objects
    	if(_size>=_capacity)
			reallocate(_capacity?_capacity*=2:++_capacity);
    	new(data+sizeof(T)*_size++) T(val);
	}
